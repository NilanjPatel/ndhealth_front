...
import {
    Grid, Card, CardHeader, Typography, List, ListItemButton,
    ListItemText, Paper, TextField, Button, Dialog,
    DialogTitle,
    DialogContent,
    DialogActions,
    Backdrop,
    Select, MenuItem, InputLabel, FormControl, CardContent, CardMedia, CardActionArea, CardActions
} from '@mui/material';

import Layout from './Layout';
import './css/Marquee.css';
import { formatTime, formatDob, formatHin, isValidEmail, formatPostalCode } from './resources/utils';
...
const ClinicPolicy = () => {
    ...
    useEffect(() => {
        const clinicData = async () => {
            if (!appointments) {
                try {
                    const response = await fetch(`${API_BASE_PATH}/terminal/${clinicSlugcurrent}/${clinic_location}/`);
    ...
    const handleAppointmentSelect1 = async (selectedAppointment) => {
        ...
        try {
            const response = await fetch(`${API_BASE_PATH}/terminal/${clinicSlugcurrent}/${selectedAppointment.appointmentId}/${dob}/`);
    ...
    const handleDoctorSelect = (doctor) => {
        ...
        const doctorAppointments = appointments.filter(appointment => appointment.doctorID__id === doctor.doctor__id);
                                        ...
                                        <List>
                                            ...
                                            {selecteddoctorAppointment.map((appointment) => (
                                                <ListItemButton
                                                    ...
                                                    selected={selectedAppointment?.appointmentId === appointment.appointmentId}
                                                    button
                                                >
                                                    <ListItemText
                            ...
                            <div>
                                <Grid >
                                    <Card style={{ backgroundColor: '#fffff', height: '3.7rem' }}>
                                        ...
                                        <Typography variant="h5" gutterBottom style={{ padding: '1rem', fontSize: '1rem', color: 'red' }}>
                                ...
                                <Card style={{ backgroundColor: '#e8eaf6' }}>
                                    <Grid item xs={12} padding={2}>
                                        <CardHeader title="Verify your self with appointment details :"
                    ...
                    {patientInfo ? (
                        <>
                            ...
                            <DialogActions>
                                ...
                                <Button onClick={close_userinfo_model} style={{ color: 'red' }} >Close</Button>
                                {!update_info && (
                                    <Button onClick={handleCloseDialog}>Update info</Button>
                                )}
