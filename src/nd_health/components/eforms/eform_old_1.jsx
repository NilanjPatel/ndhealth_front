  ...
  const renderFormItems = (items) => {
    ...
    if (Array.isArray(items)) {
      return items.map((item, index) => {
        return <div key={index}>{/* Render item content here */}</div>;
      });
    } else {
    ...
    return (
      <div key={section["@ref"]}>
        {section.items &&
          ...
          Object.keys(section.items.section).map((key, index) => (
            <div key={index}>
        ...
        {Array.isArray(section.items)
          ? section.items.map((itemOrSection, index) => (
              <div key={itemOrSection["@ref"] ?? index}>
                {itemOrSection.item
                  ? renderFormField(itemOrSection.item)
                  : renderSection(itemOrSection.section)}
              </div>
            ))
          : Object.keys(section.items).map((key, index) => (
              <div key={key}>{renderSection({ "@ref": key, items: section.items[key] })}</div>
            ))}
      </div>
    );
  };
